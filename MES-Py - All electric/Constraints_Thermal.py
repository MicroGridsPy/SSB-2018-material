...
def Solar_Energy(model,i,t): # Energy output of the solar panels
    ...
    return model.Total_Energy_PV[i,t] <= model.PV_Nominal_Capacity[i,t]*model.Inverter_Efficiency*model.PV_Units
...
def State_Of_Charge_Tank (model,i,c,t): # State of Charge (SOC) of the thermal storage (Tank)
    ...
    if t==1: # SOC_Tank  for the period 0 is equal to the Tank size.
        return model.SOC_Tank[i,c,t] == model.Tank_Nominal_Capacity[c]*1 + model.Total_Energy_SC [i,c,t] - model.Energy_Tank_Flow_Out[i,c,t] 

    if t>1:  
        return model.SOC_Tank [i,c,t] == model.SOC_Tank[i,c,t-1]*model.Tank_Efficiency + model.Total_Energy_SC [i,c,t] - model.Energy_Tank_Flow_Out[i,c,t] 
...
def Thermal_Energy_Balance(model,i,c,t): # Thermal energy balance
     ...
     :param model: Pyomo model as defined in the Model_creation library.
     '''
     return  model.Total_Thermal_Energy_Demand[i,c,t] == model.Boiler_Energy[i,c,t]  + model.Energy_Tank_Flow_Out[i,c,t] - model.Thermal_Energy_Curtailment[i,c,t] + model.Lost_Load_Th[i,c,t] + model.Resistance_Thermal_Energy[i,c,t]
